...
def get_lists(prefix_name):
    ...
    return [l for l in response.get("result", []) if l["name"].startswith(prefix_name)]
...
def get_rules(rule_name_prefix):
    ...
    return [r for r in response.get("result", []) if r["name"].startswith(rule_name_prefix)]
...
def get_list_items(list_id):
    ...
    return [i["value"] for i in response.get("result", [])]
